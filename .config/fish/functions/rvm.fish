# Make rvm work in fish. From
# https://github.com/lunks/fish-nuggets/blob/ee09ddfb4f7efac195be0957051cd1f47f00c5c2/functions/rvm.fish
function rvm --description='Ruby enVironment Manager'
    # run RVM and capture the resulting environment
    set --local env_file (mktemp -t rvm.fish.XXXXXXXXXX)
    bash -c 'source ~/.rvm/scripts/rvm; rvm "$@" 2>/dev/null; status=$?; env > "$0"; exit $status' $env_file $argv
